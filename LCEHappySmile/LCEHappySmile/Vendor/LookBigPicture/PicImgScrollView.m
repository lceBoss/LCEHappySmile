...
//        float num = (float)receivedSize/(float)totalSize;
//        _loadProgressView.text = [NSString stringWithFormat:@"%.f%@",num*100,@"%"];
        ...
        _bigImgView.image = imageScale;
        _bigImgView.bounds = CGRectMake(0, 0, imageScale.size.width, imageScale.size.height);
        [self setContentSize:CGSizeMake(_bigImgView.frame.size.width, _bigImgView.frame.size.height)];
        
        //判断图片是不是超出屏幕
        if (_bigImgView.image.size.height>HEIGHT) {
